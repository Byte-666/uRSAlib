/*____________________________________________________________________________
	...
	$Id: pgpRSAGlue.c,v 1.11 2003/12/02 22:50:01 ajivsov Exp $
____________________________________________________________________________*/
/*
...
#if __GNUC__ > 1 && !defined(PGP_UNIX_DARWIN)
 ...
 * The limit is currently set to be close to the latest 
 * equivalent strength of 3DES
 */
int
rsaKeyTooBig(RSApub const *pub, RSAsec const *sec)
{
	if (pub) {
		if (bnBits(&pub->n) > MAX_RSA_MOD_BITS)
			return MAX_RSA_MOD_BITS;
	}
	if (sec) {
		if (bnBits(&sec->n) > MAX_RSA_MOD_BITS)
			return MAX_RSA_MOD_BITS;
	}
	return 0; /* OK */
}
...
rsaPublicEncrypt(BigNum *bn, PGPByte const *in, unsigned len,
	RSApub const *pub, PGPRandomContext const *rc, PGPByte padtype)
{
	unsigned bytes = (bnBits(&pub->n)+7)/8;
	pgpPKCSPack(bn, in, len, padtype, bytes, rc);
...
bndPut("decrypted = ", bn);
	...
	return pgpPKCSUnpack(buf, len, bn, padtype, bytes);
}
